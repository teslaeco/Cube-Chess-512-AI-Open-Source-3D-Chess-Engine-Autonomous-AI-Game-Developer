...
describe("browser presentation data", () => {
  ...
  it("generates level A and all cube addresses", () => { const cube = createCubeSquareAddresses(); expect(createLevelSquares(0)).toHaveLength(64); expect(cube).toHaveLength(512); expect(new Set(cube.map((square) => square.square3D)).size).toBe(512); expect(cube[0].square3D).toBe("A:a1"); expect(cube.at(-1).square3D).toBe("H:h8"); });
