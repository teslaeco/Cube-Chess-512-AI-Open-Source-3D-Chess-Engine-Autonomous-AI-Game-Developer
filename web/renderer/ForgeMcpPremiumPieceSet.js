...
const PREMIUM_REVISION = "2026-08-31-premium-v2";
...
function addPremiumBase(group, materials, footprint = 1) {
  group.add(lathe([
    [0.0,0.0],[0.34*footprint,0.0],[0.47*footprint,0.045],[0.51*footprint,0.09],
    [0.49*footprint,0.14],[0.42*footprint,0.19],[0.37*footprint,0.24],[0.35*footprint,0.31],
  ], materials.primary));
  group.add(lathe([[0.34*footprint,0.0],[0.37*footprint,0.025],[0.35*footprint,0.055]], materials.accent, 0.13, "accent"));
  const ring = mesh(new THREE.TorusGeometry(0.405*footprint,0.027,TORUS_RADIAL,TORUS_TUBULAR), materials.secondary, 0.215, "trim");
...
function createPawn(materials) {
  ...
  group.add(lathe([
    [0.31,0],[0.29,0.05],[0.24,0.12],[0.20,0.25],[0.165,0.45],[0.17,0.56],[0.23,0.63],[0.25,0.68],[0.205,0.73],
  ], materials.primary, 0.27));
  addFacetBand(group,materials.secondary,0.245,0.96,0.075,12);
  const neckRing = mesh(new THREE.TorusGeometry(0.205,0.026,10,36),materials.accent,1.015,"accent");
  neckRing.rotation.x=Math.PI/2;
  group.add(neckRing);
  const head=mesh(new THREE.IcosahedronGeometry(0.235,2),materials.primary,1.255,"head");
  head.scale.y=1.04;
  group.add(head);
  group.add(mesh(new THREE.IcosahedronGeometry(0.073,1),materials.accent,1.49,"accent"));
  return group;
}
...
function createRook(materials) {
  ...
  group.add(lathe([[0.31,0],[0.29,0.08],[0.25,0.18],[0.225,0.43],[0.22,0.67],[0.26,0.78],[0.32,0.84]],materials.primary,0.28));
  addFacetBand(group,materials.secondary,0.29,0.83,0.08,12);
  const ring=mesh(new THREE.TorusGeometry(0.325,0.035,10,36),materials.accent,1.095,"accent");
  ring.rotation.x=Math.PI/2;
  group.add(ring);
  ...
  for(let index=0;index<8;index+=1){
    ...
    const block=mesh(new THREE.BoxGeometry(0.17,0.23,0.17,2,2,2),materials.primary,1.39,"battlement");
    block.position.set(Math.cos(angle)*0.285,1.39,Math.sin(angle)*0.285);
...
function createBishop(materials) {
  ...
  group.add(lathe([[0.30,0],[0.27,0.07],[0.22,0.16],[0.18,0.34],[0.145,0.58],[0.17,0.72],[0.235,0.80],[0.27,0.85],[0.215,0.91]],materials.primary,0.28));
  addFacetBand(group,materials.secondary,0.235,1.13,0.07,12);
  const mitre=mesh(new THREE.OctahedronGeometry(0.31,2),materials.primary,1.43,"mitre");
  mitre.scale.set(0.82,1.34,0.82);
  mitre.rotation.y=Math.PI/4;
  group.add(mitre);
  const cut=mesh(new THREE.BoxGeometry(0.075,0.72,0.52,2,6,4),materials.dark,1.46,"bishop-cut");
  cut.rotation.z=0.62;
  cut.rotation.y=0.17;
  group.add(cut);
  group.add(mesh(new THREE.IcosahedronGeometry(0.065,1),materials.accent,1.82,"accent"));
  return group;
}
...
function createQueen(materials) {
  ...
  group.add(lathe([[0.31,0],[0.28,0.07],[0.23,0.16],[0.18,0.39],[0.145,0.68],[0.17,0.80],[0.245,0.88],[0.29,0.94],[0.25,1.0]],materials.primary,0.28));
  const collar=mesh(new THREE.TorusGeometry(0.27,0.045,10,40),materials.secondary,1.25,"collar");
  collar.rotation.x=Math.PI/2;
  group.add(collar);
  const crownRing=mesh(new THREE.TorusGeometry(0.255,0.05,10,40),materials.accent,1.43,"accent");
  ...
  for(let index=0;index<10;index+=1){
    ...
    point.position.set(Math.cos(angle)*0.245,1.62,Math.sin(angle)*0.245);
    point.rotation.z=-Math.cos(angle)*0.08;
    point.rotation.x=Math.sin(angle)*0.08;
    group.add(point);
    const jewel=mesh(new THREE.IcosahedronGeometry(0.045,1),materials.accent,1.82,"jewel");
    jewel.position.set(Math.cos(angle)*0.245,1.82,Math.sin(angle)*0.245);
    group.add(jewel);
  }
  group.add(mesh(new THREE.IcosahedronGeometry(0.105,2),materials.accent,1.66,"orb"));
  return group;
}
...
function createKing(materials) {
  ...
  group.add(lathe([[0.32,0],[0.29,0.07],[0.24,0.17],[0.19,0.43],[0.16,0.73],[0.19,0.84],[0.26,0.92],[0.31,0.98],[0.27,1.04]],materials.primary,0.28));
  const shoulder=mesh(new THREE.TorusGeometry(0.29,0.052,10,40),materials.secondary,1.31,"shoulder");
  ...
  group.add(crownBase);
  group.add(mesh(new THREE.IcosahedronGeometry(0.14,2),materials.accent,1.70,"orb"));
  const vertical=mesh(new THREE.BoxGeometry(0.105,0.53,0.105,2,5,2),materials.primary,2.03,"cross");
  const horizontal=mesh(new THREE.BoxGeometry(0.43,0.105,0.105,5,2,2),materials.primary,2.06,"cross");
...
function createKnight(materials) {
  const group=new THREE.Group();
  addPremiumBase(group,materials,1.0);
  group.add(lathe([[0.30,0],[0.27,0.08],[0.23,0.2],[0.22,0.36],[0.29,0.48],[0.32,0.54]],materials.primary,0.28,"pedestal"));
  const neckCurve=new THREE.CatmullRomCurve3([
    new THREE.Vector3(0,0.77,0),new THREE.Vector3(-0.06,0.96,0.01),new THREE.Vector3(-0.14,1.17,0.03),
    new THREE.Vector3(-0.10,1.39,0.05),new THREE.Vector3(0.02,1.55,0.07),
  ]);
  const neck=mesh(new THREE.TubeGeometry(neckCurve,52,0.21,18,false),materials.primary,0,"neck");
  neck.scale.z=0.9;
  group.add(neck);
  const chest=mesh(new THREE.IcosahedronGeometry(0.30,2),materials.secondary,0.82,"chest");
  chest.scale.set(0.86,1.08,0.78);
  chest.position.x=-0.02;
  group.add(chest);
  const head=mesh(new THREE.IcosahedronGeometry(0.285,2),materials.primary,1.62,"head");
  head.scale.set(1.16,0.82,0.82);
  head.position.x=0.12;
  head.rotation.z=-0.18;
  group.add(head);
  const muzzle=mesh(new THREE.CapsuleGeometry(0.13,0.28,8,16),materials.secondary,1.54,"muzzle");
  ...
  for(const side of [-1,1]){
    const eye=mesh(new THREE.IcosahedronGeometry(0.035,1),materials.dark,1.72,"eye");
...
export class ForgeMcpPremiumPieceSet {
  ...
  buildTemplate(type,color){
    ...
    // Fit a second time so decorative outlines also remain within the real cell envelope.
    fitPieceInsideCell(group,type);
