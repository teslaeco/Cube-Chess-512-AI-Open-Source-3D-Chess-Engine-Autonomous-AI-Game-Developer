...
// Archived procedural models stay available during the staged FBX rollout.
...
export class PieceGeometryFactory {
  ...
  createArchivedProcedural(type, color) {
    const material = this.materials[color];
    const accent = this.accents[color];
    const builders = {
      pawn: () => createPawn(material),
      rook: () => createRook(material),
      knight: () => createKnightFallback(material, accent),
      bishop: () => createBishop(material, this.bishopCuts[color]),
      queen: () => createQueen(material, accent),
      king: () => createKing(material, accent),
    };
    const piece = fitPieceInsideCell((builders[type] ?? builders.pawn)());
    addOutline(piece, color === "white" ? 0x202733 : 0xc8d3df);
    return piece;
  }

  create(type, color) {
    // Stage 1: only pawns use the supplied FBX. Other pieces stay on the archived
    // procedural renderer until the pawn model is visually verified in production.
    if (type === "pawn") return this.originalModels.create(type, color);
    return this.createArchivedProcedural(type, color);
  }
}
